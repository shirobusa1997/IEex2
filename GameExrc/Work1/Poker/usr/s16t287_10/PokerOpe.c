...
void analyze_cardInfo(int *myhd, T_Info *t_i, T_ANLZInfo *numIndex, T_ANLZInfo *symIndex, int size){
  // メンバ宣言
  int i;                                      // カウンタ変数
  int tmp;                                    // テンポラリ変数
...
int judgeFrash(int *myhd, T_Info *t_i, T_ANLZInfo *symIndex, int *myud, int us){
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (symIndex[i].count == 5) { return -1; }
      ...
      if (length > 0) { decide_by_analizedDT(myhd, trash, length, myud, us, 1); }
    }
  }
...
int searchTrash(int *myhd, T_Info *t_i, T_ANLZInfo *numIndex, int s_combo, int e_combo, int *myud, int us) {
  ...
  if (length > 0) { return decide_by_analizedDT(myhd, trash, length, myud, us, 0); }
...
int decide_trash_hand(int *myhd, T_Info *t_i, T_Info *hd_res, T_ANLZInfo *symIndex, T_ANLZInfo *numIndex, int *myud, int us){
  // メンバ変数宣言
...
int strategy(int hd[], int fd[], int cg, int tk, int ud[], int us)
{
  ...
  // 捨てるカードの選択
  return_hand = decide_trash_hand(myhd, t_i, hd_res, symIndex_c, numIndex_c, myud, us);
